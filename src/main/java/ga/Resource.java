...
public class Resource {
    ...
    public List<AcquiredResource> getAcquiredResource() {
        return acquiredResourceList;
    }

    List<AcquiredResource> acquiredResourceList = new ArrayList<>();
    ...
    public boolean use(Activity activity, int quantity,int timeUnitCounterItIsLockedAt, int timeUnitsNeeded) {
        ...
        if (this.quantity >= quantity) {
            ...
            AcquiredResource acquiredResource = new AcquiredResource();
            acquiredResource.setQuantityAcquired(quantity);
            acquiredResource.setTimeUnitCounterItIsLockedAt(timeUnitCounterItIsLockedAt);
            acquiredResource.setTimeUnitsNeeded(timeUnitsNeeded);
            acquiredResource.setActivity(activity);
            this.acquiredResourceList.add(acquiredResource);
            this.quantity -= quantity;
            return true;
        } else {
    ...
    public void release(int timeUnitsTaken) {
        ...
        if(!acquiredResourceList.isEmpty()) {
            List<AcquiredResource> resourcesToBeRemovedFromQueue = new ArrayList<>();
            for(AcquiredResource resource: acquiredResourceList) {
            ...
            if(!resourcesToBeRemovedFromQueue.isEmpty()) {
                for(AcquiredResource resource:resourcesToBeRemovedFromQueue) {
                    acquiredResourceList.remove(resource);
                }
            }
        }
    }
